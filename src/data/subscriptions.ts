...
export const getSubscription = (id: string) => {
    const subscription = fetch(`${BASE_URL}subscriptions/${id}`, { headers: userHeaders })
...
export const getSubscriptions = () => {
    const subscriptions = fetch(`${BASE_URL}subscriptions`, { headers: userHeaders })
        .then(response => response.json())
        .then(data => data)
...
export const createBillingPortalSession = async () => {
    const response = await fetch(
      `${BASE_URL}subscriptions/billing_portal`,
      {
        method: "POST",
        headers: userHeaders,
        body: JSON.stringify({}),
