...
const NewBoard: React.FC = (props: any) => {
  ...
  return (
    <IonPage id="new-board-page">
      ...
      <IonButton className="" expand='block' onClick={handleCreateFromScratch} ref={scratchBtnRef}>
            Create from scratch
          </IonButton>
        <form className="ion-padding hidden" onSubmit={handleSubmit(onSubmit)} ref={scratchFormRef}>
          <IonInput aria-label="Name" placeholder="Name" defaultValue="" {...register("name", { required: true })} />
          <IonButton className="" type="submit" expand='block'>
            Create
          </IonButton>
        </form>

        <IonButton routerLink="/scenarios/new" expand="block" ref={scenarioBtnRef}>
